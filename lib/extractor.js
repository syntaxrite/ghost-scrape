  ...
  for (const node of nodes) {
    ...
    try {
      ...
      for (const candidate of candidates) {
        ...
        const headline =
          asJsonValueText(candidate.headline) ||
          asJsonValueText(candidate.name) ||
          "";

        const body =
          asJsonValueText(candidate.articleBody) ||
          asJsonValueText(candidate.text) ||
          asJsonValueText(candidate.description) ||
          "";
        ...
        return {
          ...
          text: body || "",
          author:
            asJsonValueText(candidate.author?.name) ||
            asJsonValueText(candidate.author) ||
            "",
          publishedAt:
            asJsonValueText(candidate.datePublished) ||
            asJsonValueText(candidate.dateCreated) ||
            "",
        };
      }
    } catch {
...
function extractNextData(doc) {
  ...
  try {
    ...
    const title =
      findFirstStringValue(data, ["headline", "title", "name"]) ||
      "";
    const text =
      findFirstStringValue(data, ["articleBody", "content", "body", "description", "text"]) ||
      "";
